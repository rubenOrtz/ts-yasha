const { playlist, playlist_params, playlist_offset, search_sort, search_filters, search, search_continuation, search_options } = require('./build/youtube_pb.js');
function binary_to_b64_no_pad(binary) {
    return Buffer.from(binary).toString('base64url');
}
function bin_b64(binary) {
    let str = binary_to_b64_no_pad(binary);
    while (str.length & 3) {
        str += '=';
    }
    return str;
...
module.exports = {
    ...
    gen_playlist_continuation(id, offset) {
        const p_offset = new playlist_offset();
        const p_params = new playlist_params();
        const p_cont = new playlist.playlist_continuation();
    ...
    gen_search_continuation(query, offset, options) {
        ...
        const s_data = new search_continuation.search_data();
        const s_filters = new search_filters();
        const s_options = new search_options();
        const s_position = new search_options.search_position();
        const s_off = new search_options.search_position.off();
        s_off.setTotal(0);
        s_off.setPage(1);
    ...
    gen_search_options(opts) {
        ...
        const filters = new search_filters();
        switch (opts.sort) {
            case 'relevance':
