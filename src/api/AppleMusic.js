...
class AppleMusicTrack extends Track {
    ...
    from(track) {
        ...
        this.setMetadata(
            ...
            this.gen_image(track.attributes.artwork.url),
        )
...
const api = new (class AppleMusicAPI {
    // ! This is an a modification of the original AppleMusic.js file from the project.
    ...
    async load() {
        ...
        if (!config) throw new SourceError.INTERNAL_ERROR(null, new Error('Missing config'))
        try {
        ...
        if (!config?.MEDIA_API?.token) throw new SourceError.INTERNAL_ERROR(null, new Error('Missing token'))
    ...
    async api_request(path, query = {}, options = {}) {
        ...
        if (queries.length) queries = '?' + queries.join('&')
        else queries = ''
    ...
    get_next(url, param) {
        ...
        if (!Number.isFinite(num)) throw new Error('Invalid next')
        return num
    }
    ...
    async list_once(type, id, offset = 0, limit = 100) {
        ...
        try {
            ...
            for (var item of playlist.data) result.push(new AppleMusicTrack().from(item))
            if (playlist.next)
                // @ts-ignore
