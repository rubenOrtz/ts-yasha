...
function get_property (array, prop) {
  ...
  for (const item of array) if (item && item[prop]) return item[prop]
  return null
}
...
class YoutubeResults extends TrackResults {
  ...
  /**
     ...
     */
  extract_tracks (list) {
    for (const video of list) if (video.videoRenderer) this.push(new YoutubeTrack().from_search(video.videoRenderer))
  }
