"use strict";
const { playlist, playlist_params, playlist_offset, search_sort, search_filters, search, search_continuation, search_options } = require('./build/youtube_pb');
...
module.exports = {
    ...
    gen_playlist_continuation(id, offset) {
        var p_offset = new playlist_offset(), p_params = new playlist_params(), p_cont = new playlist.playlist_continuation(), p = new playlist();
    ...
    gen_search_continuation(query, offset, options) {
        var s_cont = new search_continuation(), s_data = new search_continuation.search_data(), s_filters = new search_filters(), s_options = new search_options(), s_position = new search_options.search_position(), s_off = new search_options.search_position.off();
        s_off.setTotal(0);
        s_off.setPage(1);
    ...
    gen_search_options(opts) {
        var options = new search(), filters = new search_filters();
        switch (opts.sort) {
            case 'relevance':
