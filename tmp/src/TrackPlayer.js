...
const ERROR_INTERVAL = 5 * 60 * 1000;
...
class TrackPlayer extends EventEmitter {
    constructor(options) {
        super();
        this.normalize_volume = false;
        this.external_encrypt = false;
        this.external_packet_send = false;
        this.last_error = 0;
        this.play_id = 0;
    ...
    init_secretbox() {
        ...
        if (this.secretbox_ready()) {
            var connection_data = this.get_connection_data(), udp = this.get_connection_udp();
            var mode;
    ...
    send(buffer, frame_size, is_silence) {
        ...
        for (var i = 0; i < subscriptions.length; i++) {
            ...
            var state = connection.state.networking.state, connection_data = state.connectionData, mode = connection_data.encryption_mode;
    ...
    start_silence_frames() {
        ...
        if (this.player && this.external_encrypt && this.secretbox_ready()) {
            var box = this.player.ffplayer.getSecretBox(), data = this.get_connection_data();
    ...
    async start() {
        if (!await this.load_streams() || !this.player)
            return;
