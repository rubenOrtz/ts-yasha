...
class SoundcloudTrack extends Track {
    ...
    from(track) {
        ...
        return this.setOwner(track.user.username, [{ url: track.user.avatar_url, width: 0, height: 0 }]).setMetadata(track.id + '', track.title, track.duration / 1000, TrackImage.from(this.get_thumbnails(track)));
    }
    ...
    extract_streams(streams) {
        for (var stream of streams) {
            ...
            if (container == 'mpeg' && !codecs)
                codecs = 'mp3';
            this.push(new SoundcloudStream(stream.url)
...
var api = new (class SoundcloudAPI {
    constructor() {
        this.Track = SoundcloudTrack;
        this.Results = SoundcloudResults;
