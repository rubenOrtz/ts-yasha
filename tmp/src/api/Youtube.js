...
class YoutubeTrack extends Track {
    ...
    from(video_details, author, streams) {
        return this.setOwner(text(author.title), TrackImage.from(author.thumbnail.thumbnails))
            .setMetadata(video_details.videoId, video_details.title, number(video_details.lengthSeconds), youtube_thumbnails(video_details.videoId))
            .setStreams(streams);
    }
    from_search(track) {
        var thumbnails;
        ...
        return this.setOwner(text(track.shortBylineText), TrackImage.from(thumbnails)).setMetadata(track.videoId, text(track.title), track.lengthText ? parse_timestamp(track.lengthText.simpleText) : -1, youtube_thumbnails(track.videoId));
    }
    from_playlist(track) {
        return this.setOwner(text(track.shortBylineText), null)
            .setMetadata(track.videoId, text(track.title), number(track.lengthSeconds), youtube_thumbnails(track.videoId))
...
const api = new (class YoutubeAPI {
    constructor() {
        this.Track = YoutubeTrack;
        this.Results = YoutubeResults;
...
class YoutubeMusicTrack extends YoutubeTrack {
    ...
    from_search(track, has_type) {
        ...
        return this.setOwner(artists.join(', '), null).setMetadata(track.playlistItemData.videoId, text(track.flexColumns[0].musicResponsiveListItemFlexColumnRenderer.text), duration, youtube_thumbnails(track.playlistItemData.videoId));
    }
    from_section(track) {
